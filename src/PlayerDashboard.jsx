...
const PlayerDashboard = ({ players = [], userRole, onSelectPlayer, onAddPlayer }) => {
  ...
  useEffect(() => {
    ...
    const fetchViews = async () => {
      // ⚡ Bolt: Parallelize dashboard queries
      // Resolves all 3 independent views concurrently to minimize network waterfall and improve dashboard load times.
      // Supabase returns { count, error } instead of throwing, making Promise.all safe from short-circuiting.
      const [viewsRes, searchViewsRes, interestRes] = await Promise.all([
        supabase.from('player_views').select('*', { count: 'exact', head: true }).eq('player_id', playerId),
        supabase.from('player_search_views').select('*', { count: 'exact', head: true }).eq('player_id', playerId),
        supabase.from('scout_interests').select('*', { count: 'exact', head: true }).eq('player_id', playerId)
      ]);

      if (!isMounted) return;

      if (!viewsRes.error) setRealViews(viewsRes.count || 0);
      if (!searchViewsRes.error) setSearchApp(searchViewsRes.count || 0);
