...
const PlayerDashboard = ({ players = [], userRole, onSelectPlayer, onAddPlayer }) => {
  ...
  useEffect(() => {
    ...
    const fetchViews = async () => {
      // ⚡ Bolt: Parallelized queries to reduce network latency
      const [viewsRes, searchRes, shortlistRes] = await Promise.all([
      ...
      if (!shortlistRes.error) setShortlistCount(shortlistRes.count || 0);
    };
