...
const PlayerDashboard = ({ players = [], userRole, onSelectPlayer, onAddPlayer }) => {
  ...
  useEffect(() => {
    ...
    const fetchViews = async () => {
      ...
      // Executing these three independent Supabase count queries concurrently using Promise.all
      // instead of sequentially. This prevents request waterfalling and reduces total wait time
      // from O(A+B+C) to O(max(A,B,C)), making the dashboard load measurably faster.
