...
const PlayerDashboard = ({ players = [], userRole, onSelectPlayer, onAddPlayer }) => {
  ...
  useEffect(() => {
    ...
    const fetchViews = async () => {
      // ⚡ Bolt Optimization: Parallelize independent queries to prevent network waterfalls
      const [viewsRes, searchRes, interestsRes] = await Promise.all([
      ...
      if (!searchRes.error) setSearchApp(searchRes.count || 0);
      if (!interestsRes.error) setShortlistCount(interestsRes.count || 0);
    };
