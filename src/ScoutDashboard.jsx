...
const ScoutDashboard = ({ players = [], onSelectPlayer }) => {
  ...
  useEffect(() => {
    ...
    const fetchScoutData = async () => {
      ...
      // Grouped independent Supabase queries into Promise.all to prevent sequential network blocking.
      // Expected Impact: Reduces dashboard network latency by ~50% (from 2 sequential requests to 1 concurrent batch).
      const [viewsRes, recentViewsRes] = await Promise.all([
