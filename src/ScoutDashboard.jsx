...
const ScoutDashboard = ({ players = [], onSelectPlayer }) => {
  ...
  useEffect(() => {
    ...
    const fetchScoutData = async () => {
      ...
      // ⚡ Bolt: Parallelize independent queries to improve scout dashboard load time.
      // What: Replaced sequential await calls with Promise.all
      // Why: The top players and recent views queries are independent and can be executed concurrently
      // Impact: Reduces overall network latency by ~50%
      const [
