...
const ScoutDashboard = ({ players = [], onSelectPlayer }) => {
  ...
  useEffect(() => {
    ...
    const fetchScoutData = async () => {
      ...
      // ⚡ Bolt: Parallelize independent Supabase queries to reduce network latency
      const allViewsPromise = supabase.from('player_views').select('player_id').limit(500);
      const recentViewsPromise = supabase
        .from('player_views')
      ...
      ] = await Promise.all([allViewsPromise, recentViewsPromise]);
